...
class EnhancedInterface:
    ...
    def buffer_operations(self):
        ...
        Operations:
        1. Transfer packets from receive_buffer to send_buffer (for intermediate nodes)
        2. Check buffer status (full/empty)
        3. Manage flow control (CHOKE signal)
        4. Clear buffers when needed
        """
        ...
        # For intermediate nodes: Move packets from receive to send buffer
        # This is done when packet needs to be forwarded
        if not self.receive_buffer.is_empty() and not self.send_buffer.is_full():
            packet = self.receive_buffer.peek()
            if packet and packet.destination != self.node_address:
                # Not destination, need to forward
                forwarded_packet = self.receive_buffer.pop()
                self.send_buffer.push(forwarded_packet)
    
    def clock_tick(self):
        """
        Simulate clock tick (CLK signal)
        Updates interface state on each clock cycle
        """
        self.pin_CLK = not self.pin_CLK  # Toggle clock
        self.clock_cycle += 1
        
        # Execute control logic on rising edge
        if self.pin_CLK:
            self.control_logic()
